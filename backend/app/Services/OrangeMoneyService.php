...
class OrangeMoneyService
{
    ...
    public function __construct()
    {
        ...
        $this->environment = config('services.orange_money.environment', 'sandbox');
    ...
    public function initiatePayment(array $data): array
    {
        ...
        ])->post("{$this->baseUrl}/orange-money/mg/v1/payments", $payload);

        if ($response->failed()) {
            Log::error('Orange Money payment initiation failed', [
                'status' => $response->status(),
                'body' => $response->body(),
            ]);
            throw new \Exception('Failed to initiate Orange Money payment');
        }
    ...
    public function getTransactionStatus(string $transactionId): array
    {
        $response = Http::withHeaders([
            'Authorization' => 'Bearer ' . $this->getAccessToken(),
            'Accept' => 'application/json',
        ])->get("{$this->baseUrl}/orange-money/mg/v1/payments/{$transactionId}");

        if ($response->failed()) {
            Log::error('Orange Money transaction status check failed', [
                'transaction_id' => $transactionId,
                'status' => $response->status(),
            ]);
            throw new \Exception('Failed to get Orange Money transaction status');
        }
    ...
    public function generateUSSDCode(string $amount, string $reference): string
    {
        ...
        return "*144*1*{$this->merchantNumber}*{$formattedAmount}*{$reference}#";
    }
    ...
    {
        $response = Http::asForm()->post("{$this->baseUrl}/oauth/v3/token", [
        ...
        if ($response->failed()) {
            Log::error('Orange Money token generation failed', [
                'status' => $response->status(),
            ]);
